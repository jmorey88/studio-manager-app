...
RSpec.describe LessonPlansController, type: :controller do
  ...
  let(:student_user) {
    create(:student, name: 'Student User', teacher_id: user.id)
  }
  ...
  let(:valid_lesson_plan_params) do
  ...
  describe 'PATCH #update' do
    it 'updates lesson plan record when authenticated' do
      ...
      new_body = 'New Body'
      patch :update, params: {
        id: lesson_plan.id, lesson_plan: { body: new_body }
      }
      expect(assigns(:lesson_plan).body).to eq(new_body)
      expect(response).to redirect_to(lesson_plans_path(student_id:
        assigns(:lesson_plan).student_id))
    end
